...
def main():
    ...
    plt.savefig(file.output('energy_f_e+plasma'), bbox_inches='tight', transparent=True)

    plt.xlim(0, 2000)

    plt.savefig(file.output('energy_f_e+plasma_xlim2000'), bbox_inches='tight', transparent=True)

    plt.xlim(0, profile.TIMESTEP_MAX)

    plt.yscale("log")

    plt.savefig(file.output('energy_f_e+plasma_log'), bbox_inches='tight', transparent=True)
